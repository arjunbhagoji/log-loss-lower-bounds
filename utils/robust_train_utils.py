...
def adv(model, device, val_loader, criterion, args, epoch=0):
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(val_loader):
            ...
            # adversarial images
            images = attack_utils.pgd_whitebox(
                model,
                images,
                target,
                device,
...
def auto(model, device, val_loader, criterion, args, epoch=0):
    ...
    result = {"top1": top1.avg, "top5":  top5.avg, "loss": losses.avg, "top1_adv": top1_adv.avg, "top5_adv": top5_adv.avg, "adv_loss": adv_losses.avg}
    return result