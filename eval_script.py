...
def main():
    ...
    parser.add_argument("--val-method", type=str, default="auto", choices=("baseline", "adv", "auto"))
    ...
    model = models.__dict__[args.arch](num_classes=args.num_classes, channels=args.channels).to(device).eval()
    ...
    # dataloaders
    train_loader, train_sampler, val_loader, val_sampler, _, _, train_transform = data.__dict__[args.dataset](args.data_dir, batch_size=args.batch_size, mode=args.mode, normalize=args.normalize, size=args.size, workers=args.workers, distributed=False, args=args)
    criterion = nn.CrossEntropyLoss()
    
    # evaluation (return a dictionary from this functions and print its key-val pairs in file)
    results_train = getattr(robust_utils, args.val_method)(model, device, train_loader, criterion, args, epoch=0)
    results_val = getattr(robust_utils, args.val_method)(model, device, val_loader, criterion, args, epoch=0)
